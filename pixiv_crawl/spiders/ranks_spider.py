...
class PixivSpider(scrapy.Spider):
    ...
    def start_requests(self):
        return [
            scrapy.Request(
                ...
                callback=self.get_post_key)
        ]

    def get_post_key(self, response):
        post_key = response.css(
            '#old-login input[name=post_key]::attr(value)').extract_first()
    ...
    def logged_in(self, response):
        ...
        yield scrapy.Request(
            self.generate_list_url(self.settings['START_DATE']),
            callback=self.parse)
    ...
    def parse(self, response):
        result = json.loads(response.body, encoding='utf8')
        for section in result['contents']:
            ...
            item['total_score'] = section['rating_count']
        ...
        if result['next']:
            url = self.generate_list_url(self.settings['START_DATE'],
                                         result['next'],
                                         self.settings['SELECT_MODE'])
    ...
    def parse_detail(self, response):
        ...
        works_display=response.css('.works_display a::attr(href)').extract()
        if  works_display:
            works_display='https://www.pixiv.net/'+works_display[0]
            l=works_display.find('mode')
            r=works_display.find('&')
            mode=works_display[l+5:r]      
        if mode=='big':
            item['work_dis']='big'
            item['url']=works_display
            yield scrapy.Request(
                item['url'],
                callback=self.parse_detail_big,
                meta={'item': item},
                headers={
                    'referer':
                    response.url,
            ...
            yield scrapy.Request(
                item['url'],
                callback=self.parse_detail_manga,
                meta={'item': item},
                headers={
                    'referer':
                    response.url,
            ...
            item['work_dis']='medium'
            img_url = response.css('._illust_modal img::attr("data-src")').extract()
            item['img_urls'] = img_url
            yield item      

    def parse_detail_manga(self, response):
        item =response.meta['item']
        img_url=response.css('.item-container img::attr(data-src)').extract()
        item['img_urls'] = img_url
        yield item
    
    def parse_detail_big(self, response):
        item =response.meta['item']
        img_url=response.css('body img::attr(src)').extract()
        item['img_urls'] = img_url      
        yield item
    ...
    def generate_detail_url(self, illust_id):
        return 'http://www.pixiv.net/member_illust.php?mode=medium&illust_id={0}'.format(
            illust_id)
